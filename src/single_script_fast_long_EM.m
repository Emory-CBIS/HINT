function [theta_new,...
    ...
    beta_change,...
    z_mode,...
    PostProbs] = run_longitudinal_EM(Y,...
    X_mtx,...
    theta,...
    beta,...
    N,...
...
for iter = 1:maxit
    ...
    beta_change   = norm(beta(:) - beta_new(:)) / norm(beta(:));
    
    theta_change = max([D_change, Tau_change, Sig1_change, A_change, miu3_change, sigma3_change, pi_change]);
    
    disp(['Theta relative change: ', num2str(theta_change)])
    disp(['Beta relative change:  ', num2str(beta_change)])
    
    theta_change(iter) = (theta_change);
    beta_change(iter) = (beta_change);
