function [A, W,A_tilde_tensor,C_tensor] = fpica_tensor2_test3(X, whiteningMatrix, dewhiteningMatrix, approach, ...
...
%  Ying's modification: perform rank-2 decomp 
...
%    end
	...
	error('Code for desired nonlinearity not found!');
      ...
      %------ tensor-ICA  rank-2 approximation  
      
      
      % de-whiting w to the original space and then perform the rank-2 approx.

   clear U_tensor D_tensor V_tensor w_dewht M_r  w_dewht;
  ...
  %    D_tensor_allIC(:,:,round)=D_tensor;
      
  %    w_dewht=kron(C_tensor(:,1,round),A_tilde_tensor(:,1,round))*D_tensor(1,1)+...
  ...
  %    w=whiteningMatrix*w_dewht;
    ...
    % modification on 10/12/07
       w_dewht=dewhiteningMatrix * w;
       M_r_1=reshape(w_dewht(1:R*grp_N(1)),[R grp_N(1)]);
       M_r_2=reshape(w_dewht(1+R*grp_N(1):R*K),[R grp_N(2)]);
       
       [U_tensor,D_tensor,V_tensor]=svd(M_r_1,0);
       A_tilde_tensor(:,1,round)=U_tensor(:,1);
       C_tensor(:,1,round)=V_tensor(:,1);
       
      w_dewht(1:R*grp_N(1))=kron(C_tensor(:,1,round),A_tilde_tensor(:,1,round))*D_tensor(1,1);
      
       [U_tensor,D_tensor,V_tensor]=svd(M_r_2,0);
       A_tilde_tensor(:,2,round)=U_tensor(:,1);
       C_tensor(:,2,round)=V_tensor(:,1);
       
      w_dewht(1+R*grp_N(1):R*K)=kron(C_tensor(:,2,round),A_tilde_tensor(:,2,round))*D_tensor(1,1);
      w=whiteningMatrix*w_dewht; 
 % Normalize the new w.
