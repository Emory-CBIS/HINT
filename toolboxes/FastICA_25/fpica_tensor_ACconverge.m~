function [A, W] = fpica_tensor_ACconverge(X, whiteningMatrix, dewhiteningMatrix, approach, ...
...
% Ying 12/04/06
      M_r=reshape(dewhiteningMatrix*w,[R K]);
      [U_tensor,D_tensor,V_tensor]=svd(M_r,0);
      A_tilde_tensor=U_tensor(:,1);
      C_tensor=V_tensor(:,1);
...
% Ying 12/04/06
    A_tilde_tensorOld=zeros(size(A_tilde_tensor));
    A_tilde_tensorOld2=zeros(size(A_tilde_tensor));
    C_tensorOld=zeros(size(C_tensor));
    C_tensorOld2=zeros(size(C_tensor));
  ...
  %------ tensor-ICA  rank-1 approximation
      
      % de-whiting w to the original space and then perform the rank-1 approx.
      w_dewht=dewhiteningMatrix * w;
       M_r=reshape(w_dewht,[R K]);
      [U_tensor,D_tensor,V_tensor]=svd(M_r,0);
      A_tilde_tensor=U_tensor(:,1);
      C_tensor=V_tensor(:,1);
      
      w_dewht=kron(C_tensor,A_tilde_tensor);
      w=whiteningMatrix*w_dewht;
      %-----------------------------------
...
%Ying 12/04/06
         A_tilde_tensorOld=A_tilde_tensor;
         C_tensorOld=C_tensor;
...
%original converging criteria based on w
%      if norm(w - wOld) < epsilon | norm(w + wOld) < epsilon

%Ying 12/04/06  new converging criteria based on A_tilde_tensor and C_tensor
       if (norm(A_tilde_tensor - A_tilde_tensorOld) < epsilon | norm(A_tilde_tensor + A_tilde_tensorOld) < epsilon) & (norm(C_tensor - C_tensorOld) < epsilon | norm(C_tensor + C_tensorOld) < epsilon)
...
%Ying 12/04/06
       if (~stroke) & (norm(A_tilde_tensor - A_tilde_tensorOld) < epsilon | norm(A_tilde_tensor + A_tilde_tensorOld) < epsilon) & (norm(C_tensor - C_tensorOld) < epsilon | norm(C_tensor + C_tensorOld) < epsilon)
...
% Ying 12/04/06
      A_tilde_tensorOld2=A_tilde_tensorOld;
      A_tilde_tensorOld=A_tilde_tensor;
      C_tensorOld2=C_tensorOld;
      C_tensorOld=C_tensor;
