function [Out1, Out2, Out3,Out4,Out5,Out6] = fastica_tensor(mixedsig, varargin)
%FASTICA - Fast Independent Component Analysis for tensor ICA
%
...
%Ying, tensor ICA
     % number of time points(original or on reduce dim) for tensoring ICA
       case 'timedim'
        R=varargin{i+1};
   
     % number of subjects for tensor ICA
       case 'numofsubj'
        K=varargin{i+1};
...
if only > 2
  ...
  [A, W,A_tilde_tensor,C_tensor] = fpica_tensor2_test3(whitesig,  whiteningMatrix, dewhiteningMatrix, approach, ...
		  numOfIC, g, finetune, a1, a2, myy, stabilization, epsilon, ...
		  maxNumIterations, maxFinetune, initState, guess, sampleSize, ...
		  displayMode, displayInterval, verbose,R,K,grp_N);
