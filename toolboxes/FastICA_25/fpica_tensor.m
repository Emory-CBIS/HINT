function [A, W,A_tilde_tensor,C_tensor] = fpica_tensor(X, whiteningMatrix, dewhiteningMatrix, approach, ...
...
%    end
   ...
   clear U_tensor D_tensor V_tensor w_dewht M_r  w_dewht;
     ...
     w_dewht=dewhiteningMatrix * w;
      ...
      w_dewht=kron(C_tensor(:,round),A_tilde_tensor(:,round))*D_tensor(1,1);
  ...
  %    A_tilde_tensor2(:,round)=U_tensor(:,2);
  %    C_tensor2(:,round)=V_tensor(:,2);

  %    w_dewht=kron(C_tensor(:,round),A_tilde_tensor(:,round))*D_tensor(1,1)+...
  %             kron(C_tensor2(:,round),A_tilde_tensor2(:,round))*D_tensor(2,2);
  %    w=whiteningMatrix*w_dewht;
   %-----------------------------------
