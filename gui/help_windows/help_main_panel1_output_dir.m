function help_main_panel1_output_dir()

    helpImages = {'help_main_panel1_output_dir_fig1.png',...
    ...
    hs = findall(0,'tag','helpMainPanel1OutputDir');
    if (isempty(hs))
        hs = add_help_main_panel1_output_dir_components;
        set(hs.fig,'Visible','on');
    else
        figure(hs);
    end
    
    function hs = add_help_main_panel1_output_dir_components
        % Add components, save handles in a struct
        hs.fig = figure('Tag','helpMainPanel1OutputDir',...
            'units', 'normalized', 'position', [0.2 0.3 0.6 0.4],...
            'MenuBar', 'none',...
            'NumberTitle','off',...
            'Name','Output Directory - Help',...
            'Resize','off',...
            'Visible','off',...
            'Color',get(0,'defaultUicontrolBackgroundColor'),...
            'WindowStyle', 'modal');
        
        helpString = fileread('help_main_panel1_output_dir_text1.txt');
        
        % Text
        textPanel = uipanel('Parent', hs.fig,...
            'units', 'normalized',...
            'Position', [0.01 0.0 0.38 1],...
            'Title','Specifying an Output Directory');
        ...
        imshow(img, 'Interpolation',"bilinear");
