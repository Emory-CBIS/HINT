...
[data.hcicadir, ~, ~] = fileparts(hintFnPath);
    ...
    function hs = addcomponents
        ...
        %% Tab 1 Part 1 - Output Folder
        % Text for specifying the analysis folder
        AnalysisFolderText = uicontrol('Parent',t1p1,'Style','text','units', 'normalized', ...
            'Position',[0.05 0.65 0.3 0.28],...
            'String','Analysis folder','FontSize',myfont,...
            'HorizontalAlignment',textalign); %#ok<NASGU>
        
        AnalysisFolderTextBox = uicontrol('Parent',t1p1,'Tag','edit1', 'units', 'normalized', ...
            'Style','edit','Position',[0.35 0.65 0.6 0.28],...
            'FontSize',myfont,...
            'HorizontalAlignment',textalign,...
            'Tag','analysisFolder',...
            'TooltipString', 'All output from your analyis will be written to this folder.',...
        ...
        outputDirectoryButton = uicontrol('Parent',t1p1,...
            ...
            'String','Browse',...
            'units', 'normalized',...
            'Position',[0.325 0.2 0.35 0.28],...
            'Callback',@output_directory_button_callback,...
            'UserData','hello'); %#ok<NASGU>
        
        t1p1PanelRatio  = t1p1.Position(3) / t1p1.Position(4);        
        outputDirectoryHelpButton = uicontrol('Parent', t1p1, ...
            'Style', 'pushbutton', ...
            'String', '?', ...
            'Units', 'Normalized', ...
            'Position', [0.9, 0.1,...
            0.13 / t1p1PanelRatio, 0.13 * t1p1PanelRatio], ...
            'Tag', 'outputDirectoryHelpButton',...
            'callback', @open_outputDirectoryHelp); %#ok<NASGU>
        
        % Text for specifying the prefix - NOT IN USE
        ...
        % Text for specifying if a session log should be kept NOT IN USE
        ...
        b1r1 = uicontrol(bg1,'Style','radiobutton',...
            ...
            'FontSize',myfont,...
            'TooltipString', 'Use this option to perform a new analysis. This will require you to provide a covariate file and a brain mask (in nifti format).',...
            'units', 'normalized',...
            'Position',[0.2 0.6 0.6 0.3]); %#ok<NASGU>
        
        RadioOptionLoadSavedAnalysis = uicontrol(bg1,'Style','radiobutton',...
            'Tag','matData',...
            'String','Load saved analysis',...
            'FontSize',myfont,'units', 'normalized',...
            'TooltipString', 'Use this option to load a previously saved analysis using the runinfo.mat file.',...
            'Position',[0.2 0.2 0.6 0.3]); %#ok<NASGU>
        
        loadDataButton = uicontrol('Parent',t1p2,...
            'Tag','loadDataButton',...
            'Style', 'pushbutton',...
            'String','Load data',...
            'CallBack',@panel_1_load_data_button_push,...
            'FontSize',myfont,...
            'units', 'normalized',...
            'Position',[0.15 0.1 0.3 0.15]); %#ok<NASGU>
        
        ModelSpecButton = uicontrol('Parent',t1p2,...
            'Style','pushbutton',...
            'String','Model Specification',...
            'Callback',@open_model_specification_window,...
            'FontSize',myfont,...
            'TooltipString', 'Select covariates, coding schemes, and interaction terms.',...
            'units', 'normalized',...
...
%         end
        ...
        t1button5 = uicontrol('Parent',t1p3,'Style','pushbutton',...
            ...
            'FontSize',9, 'Position',[0.25 0.25 0.5 0.15]); %#ok<NASGU>
        %'FontSize',myfont, 'Position',[65 45 150 25]);
        
        PreviewICButton = uicontrol('Parent',t1p3,...
            'Style','pushbutton',...
            'String','Preview Population-Level Components',...
            'Callback', @preview_initial_guess,...
            'Units', 'normalized',...
            'FontSize', 9, 'Position',[0.25 0.05 0.5 0.15]); %#ok<NASGU>
...
%         %'FontSize',myfont,'Position',[120 5 150 25]);
        ...
        t2edit1 = uicontrol('Parent',t2p1,'Style','edit',...
            ...
            'String','100',...
            'HorizontalAlignment',textalign,...
            'TooltipString', 'EM Algorithm will terminate after this many iterations, regardless of convergence. Default is 100.',...
            'Tag','maxIter',...
            'BackgroundColor','white'); %#ok<NASGU>
        % Epsilon 1 Selection
        ...
        t2edit2 = uicontrol('Parent',t2p1,'Style','edit',...
            ...
            'String','0.001',...
            'HorizontalAlignment',textalign,...
            'TooltipString', 'Convergence criteria for all model parameters except the covariate effects. If the relative change is less than this number, then this component of the convergence criteria is satisfied.',...
            'Tag','epsilon1',...
            'BackgroundColor','white'); %#ok<NASGU>
        % Epsilon 2 Selection
        ...
        t2edit3 = uicontrol('Parent',t2p1,'Style','edit',...
            ...
            'String','0.1',...
            'HorizontalAlignment',textalign,...
            'TooltipString', 'Convergence criteria for the covariate effect parameters. If the relative change is less than this number, then this component of the convergence criteria is satisfied.',...
            'Tag','epsilon2',...
            'BackgroundColor','white'); %#ok<NASGU>
        ...
        t3text2 = uicontrol('Parent',t3p,'Style','text',...
            ...
            'HorizontalAlignment','right'); %#ok<NASGU>
        t3edit3 = uicontrol('Parent',t3p,...
            'Style', 'popupmenu', ...
            'units', 'normalized',...
            'Position',[0.4 0.8 0.30 0.07],...
            'FontSize',myfont,...
            'HorizontalAlignment',textalign,...
            'String', {'Input analysis path'},...
            'Tag','displayPrefix',...
            'enable', 'off'); %#ok<NASGU>
...
% run an analysis
    function panel_1_load_data_button_push(~,~)
...
%             data.N, data.q, data.X, data.Ytilde, data.hcicadir, data.nVisit);
    ...
    function preview_initial_guess(~, ~)
...
%             uiwait()
            ...
            data.variableCodingInformation = struct()
            data.variableCodingInformation.varInModel = data.varInModel;
            data.variableCodingInformation.interactionsBase = data.interactionsBase;
            cm = zeros(length(data.covariateNames), 1);
            sds = zeros(length(data.covariateNames), 1);
            for p = 1:length(data.covariateNames)
                if data.covTypes(p) == 0
                    cm(p) = mean(data.covariates{:, p});
                    sds(p) = std(data.covariates{:, p});
                end
            end
            data.variableCodingInformation.covariateMeans = cm;
            data.variableCodingInformation.covariateSDevs = sds;
            data.variableCodingInformation.effectsCodingsEncoders = data.effectsCodingsEncoders;
       
            displayResults(data.q, data.outpath, data.prefix, data.varNamesX, data.covTypes,...
                data.interactions, data.nVisit, data.validVoxels, data.voxSize,...
                data.covariateNames, data.covariates, data.variableCodingInformation, true,...
                data.maskOriginator);
...
% Run the selected algorithm.
    function runButton_Callback(~, ~)
        ...
        if data.iniGuessComplete == 1
            ...
            % Save results + variance estimates
            save_analysis_results(analysisPrefix, data);
...
% Switch the display tab to the results display tab.
    function switchDisplayTab(~,~)
        ...
        runinfofiles = dir( fullfile(data.outpath, folderName{1}, '*_runinfo.mat') );
        set( findobj('tag', 'displayPrefix'), 'string', {runinfofiles.name});
        set( findobj('tag', 'displayPrefix'), 'enable', 'on');
    end
...
% finishes.
    function stopButtonCallback(~,~)
        ...
        %msgbox('EM Algorithm terminated early.', 'modal')
    end
...
% Function to point to display path
    function browse_display_path(~,~)
        ...
        if ( ~isempty(runinfofiles) )
            set(findobj('tag', 'displayPrefix'), 'enable', 'on');
            set(findobj('tag', 'displayPrefix'), 'Value', 1);
            set(findobj('tag', 'displayPrefix'), 'String', {runinfofiles.name});
            set(findobj('tag', 'displayButton'), 'enable', 'on');
        else
            errordlg('No runinfo file found for the analysis!')
            set(findobj('tag', 'displayPrefix'), 'enable', 'off');
            set(findobj('tag', 'displayPrefix'), 'Value', 1);
            set(findobj('tag', 'displayPrefix'), 'String', {'Input analysis path'});
            set(findobj('tag', 'displayButton'), 'enable', 'off');
        end
...
% Function to open the display viewer. Belongs to Panel 3.
    function displayCallback(~,~)
        ...
        runinfoNames = get(findobj('tag', 'displayPrefix'), 'String');
        selectedFile = runinfoNames{get(findobj('tag', 'displayPrefix'), 'Value')};
        folderPath = get(findobj('tag', 'displayPath'), 'string');
        runinfo = load(fullfile(folderPath, selectedFile));
        
        % Add some useful information to the runinfo file
        runinfo.variableCodingInformation.varInModel = runinfo.varInModel;
        runinfo.variableCodingInformation.interactionsBase = runinfo.interactionsBase;
        covariateNames = runinfo.covariates.Properties.VariableNames;
        covariates = runinfo.covariates;
        cm = zeros(length(covariateNames), 1);
        sds = zeros(length(covariateNames), 1);
        for p = 1:length(covariateNames)
            if runinfo.covTypes(p) == 0
                cm(p) = mean(runinfo.covariates{:, p});
                sds(p) = std(runinfo.covariates{:, p});
            end
        end
        runinfo.variableCodingInformation.covariateMeans = cm;
        runinfo.variableCodingInformation.covariateSDevs = sds;
        
        displayResults(runinfo.q, runinfo.outfolder, runinfo.prefix,...
            runinfo.varNamesX, runinfo.covTypes,...
            runinfo.interactionsBase, runinfo.nVisit, runinfo.validVoxels,...
            runinfo.voxSize, covariateNames, covariates,...
            runinfo.variableCodingInformation, false, runinfo.maskOriginator);
...
% Save the run infoformation into a file called runinfo.mat.
    function saveContinueButton_Callback(~,~)
        ...
        % Write out the runinfo file
        fname = fullfile(data.outpath, [analysisPrefix '_runinfo.mat']);
        save_runinfo_file(fname, analysisPrefix, data)
...
% Open the window to allow the user to view the covariate files.
    function open_model_specification_window(~,~)
        if (data.dataLoaded == 1)
            ...
            [data.covariateNames, data.varInModel,...
                data.covTypes, data.effectsCodingsEncoders, data.covariates,...
                data.interactionsBase, data.weighted, data.unitScale, data.X,...
                data.varNamesX] =...
                model_specification_window(data.covariateNames, data.varInModel,...
                    data.covTypes, data.effectsCodingsEncoders, data.covariates,...
                    data.interactionsBase, data.weighted, data.unitScale, data.X,...
                    data.varNamesX);
        
            %disp('TODO - check if user changed setup after preprocessing performed - do by looking at X')
...
%% Help Functions
    ...
    function open_outputDirectoryHelp(~, ~)
        help_main_panel1_output_dir;
    end
